...
export async function saveSetting(chatId: string, key: keyof BotSettings, value: number): Promise<void> {
...
export async function loadSettings(chatId: string): Promise<BotSettings> {
  try {
    const res = await db.query(
      'SELECT trade_size_sol, take_profit_pct, stop_loss_pct FROM bot_settings WHERE chat_id = $1',
      [chatId]
    );
